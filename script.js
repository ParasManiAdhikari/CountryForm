var targetInput = document.getElementById('uni'),
    results = document.getElementById('autocomplete-results'),
    UniList = ['Albania', 'Colombia', 'Cuba', 'El Salvador', 'Jordan', 'Kenya', 'Nepal', 'Romania', 'Sri Lanka', 'Wales'],
    matches = [],
    resultsCursor = 0;
...
function getMatches(inputText) {
    ...
    for (var i = 0; i < UniList.length; i++) {
        if (UniList[i].toLowerCase().indexOf(inputText.toLowerCase()) != -1) {
            matchList.push(UniList[i]);
        }
    }
